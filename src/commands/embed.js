...
module.exports = class EmbedCommand extends Command {
    ...
    async execute(interaction) {
        ...
        var { color, field_inline_1, field_inline_2, field_inline_3 } = customEmbed;
        ...
        if (author) embed.setAuthor({ name: author, url: author_url, iconURL: author_icon });
        if (title) embed.setTitle(title);
        if (description) embed.setDescription(description);

        if (field_name_1 && field_value_1) embed.addField(field_name_1, field_value_1, field_inline_1);
        ...
        if (footer) embed.setFooter({ text: footer, iconURL: footer_icon });
