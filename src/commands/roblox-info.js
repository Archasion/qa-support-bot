...
module.exports = class RobloxInfoCommand extends Command {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            var inGroups = Object.keys(groups).length;
            var agknowledgements = [];
            var testerInGroups = 0;

            const accountCreationTimestamp = parseInt(Date.parse(info.joinDate) / 1000);

            if (info.displayName !== info.username) info.displayName += ` (@${info.username})`;

            // Add known group ranks to agknowledgements
            for (const group of groups) {
                if (group.Role.match(/(q\/?a|test)/gi)) testerInGroups++;
